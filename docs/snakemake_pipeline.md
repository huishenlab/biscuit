...
In (approximate) running order:
  ...
  - [default off] Trim adapters and/or hard clip read 2
  - Run fastq_screen on FASTQ files
  - Align, duplicate mark, sort, index, and run `samtools flagstat` on BAM (either with biscuitBlaster v1 or v2)
  - Create pileup VCF
...
## Running the Workflow
  ...
  5. Run the first rule of Snakemake on the command line.
    ...
    - Collects the list of comma separated R1 and R2 files in `bin/samples.tsv` and renames them
      - Only takes a few seconds per sample, and allows quick debugging of missing input files
